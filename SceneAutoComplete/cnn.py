...
#1) load text
print('loading text...')
train_text = json.load(open('train_preds.json', "r"))
...
for it in range(n_iteration):
    ...
    acc = (pred_e == gt).sum() / pred_e.shape[0]
    p = precision_score(gt, pred_e)
    r = recall_score(gt, pred_e)
    f = f1_score(gt, pred_e)

    # log the accuracy of subject, relation and object predictions
    logger.add_scalar('train/acc', acc, it)
    logger.add_scalar('train/precision', p, it)
    logger.add_scalar('train/recall', r, it)
    logger.add_scalar('train/f1', f, it)
