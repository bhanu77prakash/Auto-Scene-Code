...
#1) load text
print('loading text...')
raw_train_text = json.load(open('../top_150_50_new/ft_train.json', "r"))
...
kwargs = {'num_workers':n_workers, 'shuffle':True,  'drop_last':False, 'pin_memory':True, 'batch_size':batch_size}
...
e_loss_model = nn.CrossEntropyLoss()
r_loss_model = nn.CrossEntropyLoss()
...
for it in range(n_iteration):
    ...
    r_loss = r_loss_model(output_v_r, target_r)
    e_loss = e_loss_model(output_v_o, target_o)
    model_loss = (r_loss + e_loss) / 2
    #compute gradients
    model_loss.backward()
    ...
    if it % print_each == 0:
        print('it:', it, 
              ' | loss', np.round(model_loss.item(), 2), 
              ' | relation_a',np.round(acc_r, 4),
            #   ' | relation_p',np.round(p_r, 4), 
            #   ' | relation_r',np.round(r_r, 4),
              ' | entity_a',np.round(acc_e, 4),
...
# torch.save(model, './ft-layout_model.pth')
