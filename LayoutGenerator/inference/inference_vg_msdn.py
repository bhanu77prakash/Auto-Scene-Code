...
#         print(output_box[0, 2::4, :2])
        ...
        print('Kunal')
        print(refine_boxes.shape)
        print(single_data[4].shape)

        # print('entring saving section')
...
#                         name=image_id, idx=dataset_idx)
    ...
    def draw_img(self, image_size, boxes, labels, label_ids, save_dir, name, idx, mode='c'):
        ...
        for i in range(len(boxes)):
            try:
                bb.add(image, boxes[i][0], boxes[i][1], boxes[i][2], boxes[i][3], str(labels[i]+'[{}]'.format(label_ids[i])), color=color[ord(labels[i][0])-ord('a')])
            except:
                print('some error in draw_img')
...
#                           layout_save=layout_path) for idx in tqdm(range(3000)))
                    ...
                    for idx in tqdm(range(3000)):
                        # print(idx)
                        try:
                            self.check_from_model(idx, dataset = dataset, model=model,layout_save=layout_path)
                        except:
                            print('some error in main')
